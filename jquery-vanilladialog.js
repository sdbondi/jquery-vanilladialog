...
(function(window, $) {
  ...
  VanillaDialog.prototype = {
    ...
    resize: function(transition) {
      if (transition) {
        this.$container.stop().animate(this.getContentRect(), (!isNaN(transition-0)) ? transition : 200);
      } else {
    ...
    setContent: function(content) {
      ...
      if (!isInDOM || !this.$container.is(':visible')) {
        ...
        this.$container.css($.extend(rect, this.options.css))
          .fadeIn(options.animateTime, function() {
            self.afterShow.call(self);
          });       
      } else {
        this.$container.children().fadeOut(options.animateTime, function() {
          $(this).remove();
          ...
          self.$container.animate(rect, options.animateTime, function() {
            self.$content.fadeIn(options.animateTime);
            self.afterShow.call(self);
          });
        });
      }
    ...
    showOverlay: function(complete) {
      ...
      this.$overlay.fadeIn(this.options.animateTime, complete);
    },
    ...
    close: function() {
      ...
      this.$container.fadeOut(options.animateTime, fnRemove);
      this.$overlay.fadeOut(options.animateTime, fnRemove);
